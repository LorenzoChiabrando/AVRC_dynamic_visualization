...
const DEF = { cluster: 0.6, charge: -55, collidePad: 12, boundary: 0.30, spacing: 1.2, settleTicks: 350 };
...
// We read the index data/models.json (written by convert.py --all), fill the
// dropdown and decide which dataset to load. The choice lives in sessionStorage so
// the URL stays clean; on change we reload the page (a full, safe rebuild). If
// models.json is missing we fall back to data/graph.json.
const SS_KEY = "organized.model";                       // sessionStorage key: chosen file
const selEl = document.getElementById("model-select");  // the toolbar dropdown

// Add one option: value = file name (e.g. "graph.json"), text = label.
...
// On change: save the choice and reload the page.
selEl.addEventListener("change", () => {
  sessionStorage.setItem(SS_KEY, selEl.value);
  location.reload();
});

// Returns a Promise with the data file to load, after filling the dropdown.
function resolveDataset() {
  return d3.json("data/models.json").then((manifest) => {
    const models = (manifest && Array.isArray(manifest.models)) ? manifest.models : [];
    if (!models.length) throw new Error("empty manifest");   // handled by the .catch fallback below

    // Fill the dropdown in manifest order (already alphabetical by label).
    models.forEach((m) => addModelOption(m.file, m.label));

    // File to show: saved choice (if still valid), then manifest default, then the first.
    ...
    // models.json missing or unreadable: historic behavior (E. coli from graph.json).
    addModelOption("graph.json", "E. coli K-12 MG1655");
    selEl.value = "graph.json";
    return "graph.json";
  });
}

// Resolve the model, then load its JSON and build the layout.
resolveDataset().then((file) => d3.json("data/" + file)).then((raw) => {
  ...
  // For each metabolite: the set of (non boundary) subsystems that use it, then count the pairs.
